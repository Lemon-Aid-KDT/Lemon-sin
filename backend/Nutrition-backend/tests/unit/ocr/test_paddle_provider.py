...
async def test_paddle_adapter_omits_unset_detection_thresholds() -> None:
    ...
    The 2026-06-13 holdout sweep recommends text_det_box_thresh=0.4 as an
    operator-tunable env value, not a hard-coded runtime default. Unset values
    keep the upstream PaddleOCR pipeline defaults and are not forwarded.
    """
...
@pytest.mark.asyncio
async def test_paddle_adapter_forwards_box_thresh_when_configured() -> None:
    """Verify the recommended 0.4 box threshold is forwarded when configured."""
