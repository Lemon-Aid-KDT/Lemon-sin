...
def build_yolo_section_dataset_gate(
    *,
    ...
    summary = {
        ...
        "input_path_hashes": {
            key: _sha256_text(str(path.expanduser())) if path is not None else None
...
def _validation_ready(
    *,
    ...
    Returns:
        True when no validation was requested, or when require-files validation
        matches the materialized image-label counts.
    """
...
def _status(
    *,
    ...
    if validation is not None and not validation_ready:
