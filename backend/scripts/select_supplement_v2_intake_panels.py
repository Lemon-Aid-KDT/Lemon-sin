...
STRICT_INTAKE = (
    "섭취방법", "복용방법", "섭취 방법", "복용 방법", "드십시오", "드세요", "드시기",
    "섭취하", "복용하", "취침", "식후", "식전", "공복", "물과 함께", "씹어",
)
STRICT_INTAKE_RE = re.compile(r"(1일\s*\d+\s*회|1회\s*\d+\s*(?:정|캡슐|포|스푼|ml|㎖)|하루\s*\d+\s*(?:회|정|캡슐|포))")
...
def build(*, crawl_root: Path, cache_path: Path, selected_manifest: Path, output: Path, max_rescan: int) -> None:
    """Re-scan non-selected intake-flagged images and emit an intake candidate manifest."""
    cache = json.loads(cache_path.read_text(encoding="utf-8"))
    selected = [json.loads(line) for line in selected_manifest.read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    ocr = pe._build_ocr(det_model="PP-OCRv5_mobile_det", rec_model="korean_PP-OCRv5_mobile_rec",
    ...
    for phash, rec in cache.items():
        ...
        cand = [im for im in rec.get("images", []) if im["signals"].get("intake") and im["index"] not in sel]
        ...
        records.append({
            ...
            "signals": im["signals"],
            "hardness": {"long_page": im["height"] >= v2.LONG_PAGE_MIN_PX, "low_signal": False,
                         "fragmented_product": True, "selection_role": "intake"},
            ...
            "annotation": {"bbox": dict.fromkeys(("intake_method", "ingredient_amounts")),
                           "structured_gt": {"intake_method": []}, "status": "pending"},
    ...
    print(json.dumps({"images_rescanned": scanned, "intake_panels_found": len(records),
                      "products_with_intake": len({r["product_dir_hash"] for r in records})}, ensure_ascii=False))
...
def main() -> None:
    ...
    a = ap.parse_args()
    build(crawl_root=a.crawl_root, cache_path=a.cache, selected_manifest=a.selected_manifest,
          output=a.output, max_rescan=a.max_rescan_per_product)
