...
│
├── food_images/                 # 음식 이미지 데이터셋
├── supplement_images/           # 영양제 이미지 데이터셋
│
└── nutrition_reference/         # 영양 분석 기준 데이터
    ├── kdris/                   # KDRIs 룩업 테이블
    │   ├── kdris_2020.csv
    │   ├── kdris_2025.csv
    │   ├── kdris_metadata.json
    │   ├── kdris_source_manifest.json
    │   ├── review/
    │   └── raw/                 # KDRIs 원본 PDF/ZIP
    ├── mfds/                    # 식약처 단위 변환 등
    │   └── unit_conversions.json
    └── nutrient/                # nutrient code, chronic priority
        ├── nutrient_codes.json
        └── chronic_nutrient_priorities.json
```

> ⚠️ 원본 이미지와 민감 데이터는 각 도메인의 `raw/` 또는 외부 Object Storage에 두고, 개인정보 원본은 절대 커밋하지 않는다.
...
```yaml
# data/nutrition_reference/kdris/kdris_metadata.json 예시
{
  "name": "한국인 영양소 섭취기준",
  "version": "KDRIs-2020",
...
```bash
# 인코딩 확인
file -i data/nutrition_reference/kdris/kdris_2020.csv
# 정상: text/csv; charset=utf-8

# BOM 제거
sed -i '1s/^\xEF\xBB\xBF//' data/nutrition_reference/kdris/kdris_2020.csv

# CRLF → LF (Windows에서 만든 경우)
dos2unix data/nutrition_reference/kdris/kdris_2020.csv
```

### Rule 3. 표준 영양소 코드 사용

영양소는 `data/nutrition_reference/nutrient/nutrient_codes.json` 의 표준 코드만 사용:
...
```json
// data/nutrition_reference/nutrient/disease_codes.json
{
  "diabetes": {
    "name_ko": "당뇨병",
...
영양제 라벨에는 다양한 단위가 등장 (mg, μg, IU, g, 정, 캡슐). 표준 단위로 변환하는 룰을 `data/nutrition_reference/mfds/unit_conversions.json` 에 정의:
...
> 의료 도메인 프로젝트의 **가장 위험한 영역**. 작업 전 반드시 [docs/Nutrition-docs/10-compliance-checklist.md §5](../docs/Nutrition-docs/10-compliance-checklist.md) 정독.
...
5. data/nutrition_reference/kdris/kdris_2020.csv 커밋
6. 변경 사항을 docs/Nutrition-docs/09-data-catalog.md §3.1 에 기록
```
...
> 📖 **상세**: [docs/Nutrition-docs/09-data-catalog.md §4](../docs/Nutrition-docs/09-data-catalog.md)
...
```
1. 새 데이터 다운로드 → 도메인별 `raw/` 또는 `nutrition_reference/kdris/raw/`
...
def validate_kdris_csv(path: Path) -> list[str]:
    ...
    Examples:
        >>> errors = validate_kdris_csv(Path("data/nutrition_reference/kdris/kdris_2020.csv"))
...
# Validates:
# - data/nutrition_reference/kdris/kdris_2020.csv
# - data/nutrition_reference/mfds/functional_ingredients.csv
# - data/nutrition_reference/nutrient/disease_codes.json
...
- [ ] `.gitignore` 에 원본 이미지/민감 데이터 경로 제외 규칙 추가 확인
...
- [`/docs/Nutrition-docs/09-data-catalog.md`](../docs/Nutrition-docs/09-data-catalog.md) — 12종 데이터·API 카탈로그
- [`/docs/Nutrition-docs/10-compliance-checklist.md`](../docs/Nutrition-docs/10-compliance-checklist.md) — 개인정보·가명정보 처리
