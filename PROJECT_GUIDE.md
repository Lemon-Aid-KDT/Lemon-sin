...
4. OCR provider adapter와 로컬 Ollama live smoke test를 추가한다.
5. 여러 장 이미지 batch upload 계약을 설계한다: batch id, per-image validation, merged OCR text, user confirmation.
6. 팀의 음식 인식 모델·영양성분 DB lookup이 준비되면 confirmed food flow에 계산 결과를 연결한다.
7. OIDC staging provider를 선택하고 discovery/JWKS/access token e2e를 검증한다.
