...
| KDCA_HEALTHINFO_TOPIC_IDS_FILE / KDCA_HEALTHINFO_TOPIC_IDS | ignored local JSON 또는 환경변수 | 질병관리청 건강정보포털 XML topic별 4자리 ID. 만성질환·공중보건 표현 경계 readiness에 사용 |
...
[4] 환경 변수 (백엔드 환경에만):
    ...
    - MFDS_API_KEY
    - MFDS_DATA_API_KEY
    - KDCA_HEALTHINFO_TOPIC_IDS_FILE
    - KDCA_HEALTHINFO_TOPIC_IDS
...
MFDS_API_KEY=...
MFDS_DATA_API_KEY=...
KDCA_HEALTHINFO_URL_TEMPLATE=
KDCA_HEALTHINFO_TOPIC_IDS_FILE=backend/Nutrition-backend/config/kdca_healthinfo_topics.local.json
KDCA_HEALTHINFO_TOPIC_IDS={}
...
| AI Agent daily coaching/chat | 구현·live 검증 완료 | `/api/v1/ai-agent/daily-coaching`, `/api/v1/ai-agent/chat`, `agent_memory` context, Q&A knowledge policy, `source_families` metadata, emergency/drug/mental-health boundary, unsupported evidence/effect/numeric claim fallback, Ollama/SGLang provider 경로. 2026-05-25 SGLang live smoke에서 daily-coaching 2회와 chatbot 1회 모두 `sglang` provider로 응답하고 memory 재주입을 확인 |
| 모바일 MVP | 연결 완료 | `mobile/flutter_app` Flutter shell, taedong-inspired bottom shell/capture frame/result screen, dashboard routing, secure token store, Dio API client, Android emulator-safe local base URL, daily-coaching DTO/repository, taedong-compatible loose models, 영양제 촬영 permission/image picker + `/api/v1/supplements/analyze` multipart preview 연결 + confirmed `/api/v1/supplements` 저장 flow, 음식 사진/수동 입력 confirmed payload, debug-only confirmed sample seeding, `ConfirmedEntryStore` 앱 세션 handoff, chatbot source family chip, 면책 고지, Python contract test, Flutter analyze/test |
...
4. `KDCA_HEALTHINFO_TOPIC_IDS_FILE` 또는 `KDCA_HEALTHINFO_TOPIC_IDS`와 필요 시 `MFDS_DATA_API_KEY`를 repo root `.env`, `backend/.env`, 실행 환경 또는 `--env-file`에 넣고 `backend/scripts/check_ai_agent_runtime_prereqs.py --require-medical-sources kdca-healthinfo mfds-drug-safety`로 `kdca-healthinfo`, `mfds-drug-safety` readiness를 종료코드까지 포함해 재확인한다.
5. KDCA topic id 반영 후 AI Agent strict source gate를 재확인하고, 필요 시 Ollama 서버 재시작 후 `backend/scripts/check_ai_agent_runtime_prereqs.py --require-ollama --require-ollama-parser-smoke`로 개발용 Ollama live parser path를 재확인한다.
