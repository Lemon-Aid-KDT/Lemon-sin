...
> 출처 메모: 필라이즈 MAU·투자 유치 수치는 시리즈 B 보도자료 기준(2024년 발표). 정확한 인용은 §22 참고 자료 참조.
...
### 17.3 시나리오 3 — 챗봇 알림 등록 (Tool Use 어필)
...
| ci-docs.yml | `markdownlint PROJECT_GUIDE.md README.md` → SPLIT 카운트 검증 → guide.html script 태그 균형 체크 |
...
⚠️  guide.html 의 HTML/CSS/JS 영역은 손대지 마세요. md-source 안의 마크다운만 수정.
⚠️  코드 시그니처(A.6)는 합의된 인터페이스. 변경 시 PR 필수.
⚠️  GitHub 협업 규칙은 §16 참조.
```
...
| 발표 직전인데 백엔드 죽음 | §17.4 / §20.5 시연 안전장치 |

---

> 본 문서는 Lemon Aid 팀 공유용 가이드라인입니다.
> 변경 시 PROJECT_GUIDE.md만 수정하면 guide.html에 자동 반영됩니다.
