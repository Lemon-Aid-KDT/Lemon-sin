...
- local 개발용 인증 방식 확정: `.env`의 `GOOGLE_CLOUD_API_KEY`
- API key를 repo, docs, fixture, issue/PR 본문에 남기지 않는 운영 규칙 확정
...
- local dev에서는 `.env`에 `GOOGLE_CLOUD_API_KEY` 값만 입력한다.
- `GOOGLE_CLOUD_API_KEY`가 비어 있는데 `OCR_PRIMARY_PROVIDER=google_vision`이면 adapter factory가 fail closed한다.
- API key는 Google Cloud Console에서 Cloud Vision API로 제한하고, 서버 env에만 둔다. 모바일/프론트 번들에는 포함하지 않는다.
...
def build_supplement_ocr_adapter(settings: Settings) -> OCRAdapter | None:
    ...
    if settings.ocr_primary_provider == "google_vision":
        ...
        return GoogleVisionOCRAdapter(api_key=settings.google_cloud_api_key, ...)
```
...
- Google API key가 URL, log, audit에 남지 않음
...
- [ ] production 인증은 API key 제한 또는 attached service account 방식 중 하나로 결정
