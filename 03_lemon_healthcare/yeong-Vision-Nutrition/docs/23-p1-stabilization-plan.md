...
| 항목 | 명령 | 현재 결과 |
| --- | --- | --- |
| 전체 backend test | `.venv/bin/python -m pytest -o addopts=''` | `211 passed, 1 skipped` |
| coverage gate | `.venv/bin/python -m pytest --cov=src --cov-report=term-missing --cov-fail-under=80 -q` | `211 passed, 1 skipped`, total coverage `85.56%` |
| formatting | `.venv/bin/black --check src tests alembic` | pass |
| lint | `.venv/bin/ruff check src tests alembic` | pass |
| type check | `.venv/bin/mypy src tests --strict` | pass |
| settings JSON parse | `.venv/bin/python -m json.tool ../config/implementation-readiness.settings.json` | pass |
| OpenAPI generation | `create_app().openapi()` | OpenAPI `3.1.0`, `20 paths` |
| AI flag defaults | `Settings()` default check | `allow_external_llm`, `enable_multimodal_llm`, `enable_vision_classifier`, `enable_image_learning_pipeline`, `enable_pgvector_storage` all `false` |
...
| test baseline drift | 새 모듈 추가 후 전체 테스트는 통과하지만 CI path와 다르게 검증될 수 있음 | 로컬 검증 명령과 `ci-backend.yml` 명령을 동일하게 유지 |
...
현재 git root는 `/Users/yeong/99_me/00_github`이고 이 프로젝트 prefix는 `03_lemon_healthcare/`다. 따라서 `.github/workflows/ci-backend.yml`의 path filter와 `defaults.run.working-directory`는 이 prefix를 유지해야 한다.

현재 backend CI path:

- trigger path: `03_lemon_healthcare/yeong-Vision-Nutrition/backend/**`
- workflow path: `03_lemon_healthcare/.github/workflows/ci-backend.yml`
- working directory: `03_lemon_healthcare/yeong-Vision-Nutrition/backend`
- Python matrix: `3.13`
- dependency file: `requirements-dev.txt`
- PostgreSQL service: `postgres:16`, `DATABASE_URL` and `TEST_DATABASE_URL` both use `postgresql+asyncpg://lemon:lemon@localhost:5432/lemon_test`
...
| 설정 | 현재 기본값 | P1 안정화 목표 |
| --- | --- | --- |
| `feature_prescription_ocr_intake` | `true` | `false` |
| `feature_lab_result_ocr_intake` | `true` | `false` |
| `feature_medication_safety_alert` | `true` | `false` |
| `FEATURE_PRESCRIPTION_OCR_INTAKE` in `implementation-readiness.settings.json` | `true` | `false` |
| `FEATURE_LAB_RESULT_OCR_INTAKE` in `implementation-readiness.settings.json` | `true` | `false` |
| `FEATURE_HOSPITAL_MOCK_FHIR` in `implementation-readiness.settings.json` | `true` | `false` |
| `FEATURE_MEDICATION_SAFETY_ALERT` in `implementation-readiness.settings.json` | `true` | `false` |

구현 시 추가할 테스트:

- development 기본값에서 모든 non-P1 feature flag가 `false`인지 확인
- production에서 AI/vision/learning flag가 true이면 validation error
...
- 전체 backend test `211 passed, 1 skipped` 이상 유지
...
- `.github/workflows/ci-backend.yml`의 path filter, working-directory, Python 3.13 matrix를 재확인한다.
- backend workflow에 settings/OpenAPI contract smoke command를 명시적으로 추가할지 결정한다.
- docs-only 변경이 backend CI를 불필요하게 트리거하지 않는지 path filter를 확인한다.

권장 추가 command:

```bash
python -c "from src.config import Settings; s=Settings(); assert not s.enable_multimodal_llm and not s.enable_vision_classifier and not s.enable_image_learning_pipeline and not s.enable_pgvector_storage"
python -c "from src.main import create_app; schema=create_app().openapi(); assert schema['openapi'].startswith('3.')"
```

현재 backend CI에는 위 설정/OpenAPI smoke와 PostgreSQL `alembic upgrade head` smoke가 포함되어 있다.
...
1. `fix(test): restore strict mypy baseline for supplement image tests`
   - 이유: P1 안정화 전 CI 기준선을 먼저 깨끗하게 만들기 위함.
2. `test(contract): freeze P1 OpenAPI route contracts`
   - 이유: 이후 AI 기능 추가가 기존 모바일/API contract를 약화하지 못하게 하기 위함.
3. `fix(config): default non-P1 feature flags to disabled`
   - 이유: 검증되지 않은 AI/규제 기능이 기본 활성화되는 것을 막기 위함.
4. `ci(backend): add P1 settings and OpenAPI smoke checks`
   - 이유: 로컬 안정화 기준이 GitHub Actions에서도 동일하게 재현되도록 하기 위함.
