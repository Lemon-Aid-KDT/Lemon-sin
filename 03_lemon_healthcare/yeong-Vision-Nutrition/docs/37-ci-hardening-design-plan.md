...
1. backend CI trigger가 backend 코드뿐 아니라 `data/kdris/**`, `data/reference/**`, `config/**` 변경도 감지한다.
...
on:
  push:
    paths:
      - '03_lemon_healthcare/yeong-Vision-Nutrition/backend/**'
...
- `data/kdris/**`: KDRIs 2025 CSV, manifest, review artifact 변경 검증
- `data/reference/**`: 만성질환 우선순위, nutrient reference 등 backend 분석 기준 변경 검증
...
- `03_lemon_healthcare/yeong-Vision-Nutrition/docs/**`
- `03_lemon_healthcare/yeong-Vision-Nutrition/**/*.md`
...
| `data/kdris/**` 변경 | markdown이면 실행 가능 | 실행 | 데이터가 backend 분석 결과에 영향 |
| `data/reference/**` 변경 | markdown이면 실행 가능 | 실행 | reference가 backend 분석 결과에 영향 |
...
- `defaults.run.working-directory`가 `03_lemon_healthcare/yeong-Vision-Nutrition/backend`인지 확인
