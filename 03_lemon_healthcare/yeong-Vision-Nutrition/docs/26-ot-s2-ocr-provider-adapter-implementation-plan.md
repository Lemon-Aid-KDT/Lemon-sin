...
세부 검토 항목과 착수 조건은 `docs/27-ot-s2b-google-vision-ocr-review-plan.md`를 기준으로 한다.
...
| `docs/dev-guides/07-ocr-pipeline.md` | 현행 provider 상태를 `ClovaGeneralOCRAdapter` 1차 구현 기준으로 업데이트. Google/CLOVA 자동 fallback 표현은 아직 미구현으로 명확히 표시. |
| `docs/dev-guides/09-supplement-registration-api.md` | `/analyze` one-shot OCR+parse preview 동작, 실패 code, empty OCR 정책 추가. |
...
```bash
git diff --check
rg -n "Google Cloud Vision.*완료|CLOVA.*자동 폴백.*완료|YoloLabelDetector|OllamaAdapter" yeong-Vision-Nutrition/docs
```
