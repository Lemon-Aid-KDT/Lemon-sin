...
class NutrientStatus(StrEnum):
    ...
    Reference:
        docs/Nutrition-docs/07-core-algorithm.md §4.3
    """
...
Reference:
    docs/Nutrition-docs/09-data-catalog.md §3.1
    data/CLAUDE.md
"""
...
DEFAULT_KDRIS_PATH: Final[Path] = Path("data/nutrition_reference/kdris/kdris_2020.csv")
...
def load_kdris_csv(path: Path = DEFAULT_KDRIS_PATH) -> list[dict]:
    ...
    Examples:
        >>> rows = load_kdris_csv(Path("data/nutrition_reference/kdris/kdris_2020.csv"))
        >>> rows[0]["code"]
        'vitamin_c_mg'
    """
    if not path.exists():
