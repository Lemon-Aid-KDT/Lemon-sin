...
| 항목 | 현재 상태 |
| --- | --- |
| 기본 DB URL | `backend/src/config.py`의 local-development PostgreSQL DSN default |
...
TEST_DATABASE_URL=postgresql+asyncpg://${DB_USER}:${DB_PASSWORD}@localhost:5432/lemon_test \
  .venv/bin/python -m pytest tests/integration/db/test_db_session.py -q

DATABASE_URL=postgresql+asyncpg://${DB_USER}:${DB_PASSWORD}@localhost:5432/lemon_test \
