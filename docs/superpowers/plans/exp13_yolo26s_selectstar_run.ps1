# exp13: YOLO26s + taxo59 + selectstar 실데이터 11클래스 확대(takoyaki 성공 일반화). baseline = exp11(noSS)·exp12(takoyaki).
# 변경점 = 11클래스 각 +800 selectstar 실데이터(takoyaki+jajangmyeon·udon·bulgogi·dim_sum·ramen·korean_pancake·kimchi_stew·sashimi·rice_noodle·bibimbap). val 동일. seed=42.
...
param(
    ...
    [int]$Epochs = 50,
    [int]$Seed = 42,
    [string]$RunName = "exp13_yolo26s_selectstar_pc1"
)
...
$dataYaml   = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_exp13_selectstar\data.yaml"
$project    = "C:\Lemon-sin\runs\food_yolo"
$fullName   = "$RunName" + "_s$Seed" + "_b$Batch" + "_w8_cache_disk_det_true"
$dsRoot     = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_exp13_selectstar"
...
Write-Host ""
Write-Host "=== exp13 학습 시작 (YOLO26s + selectstar 11클래스 확대, seed=$Seed) ===" -ForegroundColor Cyan
