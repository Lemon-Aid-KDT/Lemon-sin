...
> **v4 변경점**: Phase A를 dev-guide 06/08과 독립적으로 진행하도록 조정. `RdaMatcher`는 Phase A에서 `FoodMatchResult` 중간 출력까지만 검증, `NutrientIntake` 변환은 06 완료 후 follow-up PR. AI Hub 다운로드는 Phase B부터. 모델 파일 보관 정책은 Beta 학습 이후 결정.
...
| `PortionEstimator` | 추정 g, 양 표현, confidence | 1인분 기본값 + bbox 비중 보정 | bbox/접시 크기 기반 보정 |
| `RdaMatcher` | 음식명/class → 농진청 food_code 매칭 | 100종 시드 | 전체 데이터 |
...
- **`RdaMatcher`는 중간 출력(`FoodMatchResult`)까지만 검증한다.** 06 완료 후 `NutrientIntake` 변환 계층을 별도 작업으로 추가.
...
#### A3 — RDA matcher + Text parser + Edge cases + Compliance polish (Integration & Polish)

> Phase A 마무리. RDA matching, 텍스트 입력, edge case 보강, 의료 표현 자동 검사로 dev-guide 16 §Definition of Done(06 의존 제외)을 모두 충족.

**스코프:**
- `RdaMatcher` + `FoodMatchResult` DTO (food_code, 1회 제공량, 영양소 row)
  ```python
...
- `pipeline.py`의 RDA stub을 실 `RdaMatcher` 호출로 교체
- Edge case 테스트 5+ 추가
...
**테스트:**
- RDA matching:
  - 매칭 성공: alias → food_code → `FoodMatchResult` 정확 (영양소 값이 CSV 1회 제공량 기준 일치)
...
- Edge cases (5+):
  ...
  - 매칭 실패 음식 → pipeline에서 항목 보존 + `needs_user_review=True`
  - 1글자 음식명 / 특수문자 포함 이름
- Compliance:
...
**커밋 단위 권장:**
1. `feat(nutrition): RdaMatcher + FoodMatchResult DTO + 단위 테스트`
2. `feat(meal): text_parser 정규화·alias 처리 + 단위 테스트`
3. `feat(meal): pipeline의 RDA stub을 실 matcher로 교체`
4. `test(meal): edge case 5+ 케이스 추가`
5. `test(meal): 의료 표현 자동 검사 + compliance polish`
6. `test(meal): 통합 테스트 (image → RecognizedMeal end-to-end)`
...
- 06 후 follow-up PR(`to_nutrient_intakes` 추가)을 위한 hook이 명시적으로 준비됨 (`pipeline.py`에 TODO 주석)
...
1. `backend/src/nutrition/rda_matcher.py`에 변환 함수 추가:
   ...
   def to_nutrient_intakes(
       matches: list[FoodMatchResult],
       actual_grams: list[float],
   ) -> list[NutrientIntake]: ...
   ```
2. `pipeline.py`의 최종 단계에서 `RecognizedMeal`의 각 item을 `actual_grams`로 스케일링해서 `NutrientIntake[]` 반환
3. 통합 테스트 추가 (사진 → 인식 → 영양소 list → 06 모듈로 진단)
...
- 음식 후보/추정량/`needs_user_review` 플래그가 일관된 스키마로 반환
- `RdaMatcher`가 `FoodMatchResult`를 정확히 반환 (영양소 값 검증)
...
- 06 후 follow-up hook 준비됨 (`pipeline.py`에 TODO 주석 + 변환 함수 시그니처 합의)
...
- **Phase A는 dev-guide 06/08과 독립적** — mock-first로 시작, `RdaMatcher`는 `FoodMatchResult`까지만 검증
- **dev-guide 16 본체는 Phase A에서 (06 의존 부분 제외) 완성** — base.py, exceptions.py, mock adapter, fusion, portion, RDA 매칭, pipeline 모두 MVP 산출물
- **06 후 후속 PR로 `NutrientIntake` 변환 통합** — Phase A 완성도와 무관한 별도 작업
...
- [ ] A2 머지 후 **A3 시작** — RDA matcher + Text parser + Edge cases + Compliance (커밋 6개, 세 번째 PR)
- [ ] Phase A (A1+A2+A3) 완료 후 Open Decisions #1, #2 확정 → Phase B 진입
- [ ] Phase B 시점에 AI Hub 계정 등록 + 50종 다운로드 시작
- [ ] dev-guide 06 완료 시 follow-up PR로 `to_nutrient_intakes(...)` 변환 통합
