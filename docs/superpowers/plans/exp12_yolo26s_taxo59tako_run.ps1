# exp12: YOLO26s + taxo59 cap1500 + takoyaki selectstar 실데이터 보강. exp11 대비 takoyaki만 +1090 실데이터. baseline = exp11.
# 변경점 = takoyaki 410->1500 (selectstar 실데이터 +1090, 모델박스+정답라벨). 다른 클래스·val·설정 exp11 동일. seed=42(exp11 직접비교).
...
param(
    ...
    [int]$Epochs = 50,
    [int]$Seed = 42,
    [string]$RunName = "exp12_yolo26s_taxo59tako_pc1"
)
...
$dataYaml   = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_exp12_takoyaki\data.yaml"
$project    = "C:\Lemon-sin\runs\food_yolo"
$fullName   = "$RunName" + "_s$Seed" + "_b$Batch" + "_w8_cache_disk_det_true"
$dsRoot     = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_exp12_takoyaki"
...
Write-Host ""
Write-Host "=== exp12 학습 시작 (YOLO26s + taxo59 +takoyaki실데이터, seed=$Seed) ===" -ForegroundColor Cyan
