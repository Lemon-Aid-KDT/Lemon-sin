# exp10: YOLO26s + taxonomy v4(taxo59) balanced. mala/탕수육/제육 _DROP. baseline = exp09.
# 변경점 = 데이터셋(taxo62->taxo59: 약점3 _DROP). 모델/설정 exp09 동일. **시드 2개**(42,123)로 run-noise 분리.
...
param(
    ...
    [int]$Epochs = 50,
    [int]$Seed = 42,
    [string]$RunName = "exp10_yolo26s_taxo59bal500_pc1"
)
...
$dataYaml   = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_bal500\data.yaml"
$project    = "C:\Lemon-sin\runs\food_yolo"
$fullName   = "$RunName" + "_s$Seed" + "_b$Batch" + "_w8_cache_disk_det_true"
$dsRoot     = "C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo59_bal500"
...
Write-Host ""
Write-Host "=== exp10 학습 시작 (YOLO26s + taxo59 v4 balanced, seed=$Seed) ===" -ForegroundColor Cyan
...
Write-Host "(yolo26s ~9.5M, batch=16. exp09 대비 데이터셋만 변수=drop3. 시드 42·123 두 번 실행해 평균/편차로 효과 확인)" -ForegroundColor DarkGray
