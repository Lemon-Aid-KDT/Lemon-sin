# exp06: YOLO11s + taxonomy v2 (63클래스) balanced(train<=500/val<=100) 학습
# 변경 변수 = 택소노미(50->63) 하나. 비교 baseline = exp03_yolo11s_balanced500_pc2 (50클래스).
# 설정은 exp03 yolo11s와 동일(b32 권장; b48은 8GB OOM 이력).

param(
    [int]$Batch = 32,
    [int]$Epochs = 50,
    [string]$RunName = "exp06_yolo11s_taxo63bal500_pc1"
)
...
if (-not (Test-Path $dataYaml)) { Write-Host "ERROR: data.yaml 없음 -> $dataYaml (balanced 빌드 먼저)" -ForegroundColor Red; exit 1 }
...
Write-Host ""
Write-Host "=== exp06 학습 시작 (YOLO11s + taxo63 balanced) ===" -ForegroundColor Cyan
...
Write-Host "(batch=48은 8GB OOM 이력 -> 기본 32. 재시도하려면 -Batch 48)" -ForegroundColor DarkGray
Write-Host ""
