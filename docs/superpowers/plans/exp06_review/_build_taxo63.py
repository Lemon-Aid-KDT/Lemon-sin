"""taxonomy v2(63클래스) 데이터셋 빌드.

424코드 라벨을 63 최종클래스로 리매핑하고, 이미지는 하드링크로 재사용한다.
_DROP 코드 라인은 제거하고, 라벨이 비면 이미지도 제외한다.

usage:
    python _build_taxo63.py --dry      # 클래스/물량만 검증, 파일 미생성
    python _build_taxo63.py            # 실제 빌드
"""
...
DST = Path(r"C:\Lemon-sin\data\food_images\processed\aihub_yolo_taxo63")
MAP_CSV = Path(r"C:\Lemon-sin\docs\superpowers\plans\exp06_review\exp06_taxonomy_v2_mapping.csv")
