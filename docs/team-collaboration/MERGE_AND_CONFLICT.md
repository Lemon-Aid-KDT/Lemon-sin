...
| **feature 브런치 최신화** | `rebase origin/develop` (merge 금지) |
...
git rebase origin/develop              # develop도 합치고
git push origin <내브런치> --force-with-lease
```
...
# 다음 작업
git checkout -b feat/<영역>-<다음주제> origin/develop
```
...
# 또는 CLI
git fetch origin
git checkout -b revert/feat-mobile-dashboard origin/develop
git revert -m 1 <머지커밋해시>
git push origin revert/feat-mobile-dashboard
...
- [ ] 매일 시작 전 `git sync` (= `git fetch origin && git rebase origin/develop`)
- [ ] PR은 작게 (≤ 500줄 권장)
- [ ] 같은 모듈을 건드리는 PR끼리 머지 순서 협의
- [ ] 머지 직전에 다시 한 번 `git fetch origin && git rebase origin/develop`
...
| `git push origin develop` (직접) | ❌ 보호 규칙이 차단 |
| `git push origin main` | ❌ 보호 규칙이 차단 |
