...
```bash
1. git fetch origin
2. git rebase origin/develop          # 충돌 발생
3. <파일 열어서 마커 정리>
4. git add <해결한 파일>
5. git rebase --continue            # 끝까지 반복
6. git push origin <브런치> --force-with-lease
```
...
- ❌ `git push origin develop` / `git push origin main` (직접)
- ❌ `.env` / 비밀키 커밋
- ❌ 본인 PR 셀프 머지
...
2. **매일 sync** — `git fetch origin && git rebase origin/develop`
