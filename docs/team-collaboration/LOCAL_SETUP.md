...
[alias]
    ...
    # develop 브런치에서 새 작업
    new = "!f() { git fetch origin && git checkout -b $1 origin/develop; }; f"
...
## 4. 보호 브런치와 Conventional Commits hook

이 저장소는 `.pre-commit-config.yaml`에서 다음 로컬 hook을 제공합니다.

- `protect-protected-branch`: `main`/`develop`에서 직접 커밋 차단
- `protect-protected-branch-push`: `main`/`develop` 직접 push 차단
- `conventional-commit-message`: Conventional Commits 제목 검증
...
```bash
python scripts/git-hooks/validate_commit_msg.py --title "docs(team): 협업 규칙 문서 추가"
python scripts/git-hooks/validate_team_policy.py --base develop --branch docs/team-collaboration-rules --title "docs(team): 협업 규칙 문서 추가"
```
...
- `detect-private-key`
- `protect-protected-branch` (`main`/`develop` 직접 커밋 차단)
- `protect-protected-branch-push` (`main`/`develop` 직접 push 차단)
- `conventional-commit-message` (커밋 메시지 검증)
