# 초기 요구 기능 기반 MVP 구현 제안서

> 목적: 초기 요구 기능을 Lemon Aid가 어떤 MVP 흐름으로 풀어갈지 설명한다.

## 1. 초기 요구 기능 이해

초기 요구 기능은 크게 두 축으로 볼 수 있다.

| 요구 축 | 핵심 내용 | Lemon Aid에서의 해석 |
|---------|-----------|----------------------|
...
두 요구 기능은 별개의 기능처럼 보이지만, 서비스 관점에서는 하나의 흐름으로 연결된다. 영양제와 음식은 모두 사용자의 섭취 데이터이고, 이 데이터를 사용자 프로필과 건강 상태에 맞게 해석해야 개인화 건강관리 서비스가 된다.

우리는 두 요구 기능을 만성질환 관리 사용자의 하루 단위 섭취·활동 데이터를 해석하는 문제로 보았다. 영양제 분석은 복용 중인 성분과 함량을 확인하는 입력이고, 음식·식단 분석은 실제 섭취와 활동 맥락을 확인하는 입력이다.

두 입력을 합쳐야 부족·과다 가능성, 목적별 분석, 활동 권고가 만성질환자와 예방 관리 사용자를 위한 개인화 건강관리 피드백으로 연결된다.

## 2. MVP 목표

MVP의 목표는 모든 기능을 완성하는 것이 아니라, 우리 팀의 서비스 방향이 실제 흐름으로 발전할 수 있음을 보여주는 것이다.
...
분석 결과는 Lemon Aid가 제안하는 서비스 방향에 맞춰 5종 출력으로 정리한다.
...
| 5종 출력 이해도 | 팀과 사용자가 결과 의미를 빠르게 이해할 수 있는가 |
...
이 흐름에서 MVP는 완성된 의료 서비스가 아니라, 초기 요구 기능이 실제 서비스로 발전할 수 있음을 보여주는 참조 모델이다.

## 7. 작성 및 발표 기준

- 기능을 많이 나열하기보다 "입력 → 분석 → 결과 → 검증" 흐름으로 설명한다.
- 초기 요구 기능과 Lemon Aid의 서비스 방향이 어떻게 연결되는지 보여준다.
