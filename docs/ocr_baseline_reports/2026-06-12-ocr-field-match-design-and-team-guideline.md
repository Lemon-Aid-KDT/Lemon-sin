...
TODO: det thresh 3종을 `config.py` + `paddle.py:_predict_kwargs`에 노출하는 구현 티켓 분리 (sweep에서 효과 확인된 값만).
...
- TODO: det thresh 3종(`text_det_thresh`/`box_thresh`/`unclip_ratio`) 런타임 배선 (`config.py` + `paddle.py:_predict_kwargs`)
