...
>
> **2026-06-13 우선 적용 규칙**: 이 확정판과 아래 두 문서가 충돌하면 2026-06-13 문서의 최신 측정값과 게이트 결정을 우선한다.
> - `outputs/todo-list/2026-06-13/2026-06-13-ocr-benchmark-required-section-decision.md`
> - `outputs/todo-list/2026-06-13/2026-06-13-section-detector-training-gate-runbook.md`
...
| teacher OCR 벤치마크 게이트 | **2026-06-12 당시 blocked_by_pii_screening** — 후보 215행 전부 PII 미결, GT 0행, holdout fixture 0. **2026-06-13 업데이트는 §0.1을 우선 적용** | `outputs/generated/supplement-learning/2026-06-05/operator-review/ocr-benchmark-gate.md` |
...
**함의(2026-06-12 기준)**: 목표(0.85/0.85)까지 격차는 field_match 기준 약 +0.30이며, 단일 레버로는 닿지 않는다. 검출기 게이트가 blocked이므로 ROI-scoped 승격 경로는 현재 닫혀 있고, full-image 기준 quick win + recognition fine-tune이 먼저 움직일 수 있는 레버다. 단, PII/GT/benchmark 상태와 b32 holdout 측정값은 §0.1의 2026-06-13 업데이트를 우선한다.

### 0.1 2026-06-13 우선 적용 업데이트

| 항목 | 최신 결정/측정값 | 적용 방식 |
|---|---|---|
| 필수 섹션 | `ingredient_amounts`, `intake_method` 핵심 2종을 필수로 채택. `precautions`, `allergen_warnings`는 존재 시 채점·부재 시 제외 | 기본 4종 필수 가정이 남아 있는 오래된 런북보다 `2026-06-13-ocr-benchmark-required-section-decision.md`를 우선한다 |
| benchmark manifest | 203 fixtures, holdout 52/test 22/train 129, product-group split, leakage check passed, raw OCR/provider payload/absolute paths false | 이후 b32/quick-win/det sweep 평가는 이 holdout 해석을 기준으로 비교한다 |
| b32 holdout | field_match macro **0.6049**, micro **0.614**(105/171), ingredient_recall **0.6842**(65/95), scored 41/failed 0 | b32는 recognition 후보일 뿐 production 승격 아님. `field_match>=0.85`, `ingredient_recall>=0.85` gate 미달 |
| det threshold sweep | `text_det_box_thresh=0.4`가 단일 노브 승자. within-sweep 기준 macro +0.0098, micro +0.0175, ingredient_recall +0.021 | 코드 기본값 하드변경이 아니라 `LOCAL_OCR_TEXT_DET_BOX_THRESH=0.4` 튜너블 권고로 적용. combined 노브는 오버슈트로 채택 금지 |
| section detector | 2026-06-09 `yolo26s`는 ROI-first가 full-image보다 나빠 임시 배선 금지. 현재 gate는 `blocked_by_annotation_review`, 미검토 bbox 205건 | `2026-06-13-section-detector-training-gate-runbook.md`를 우선 적용. 205 bbox 리뷰·라벨 보강·재학습 전까지 full-image fallback 유지 |
...
| 항목 | 정책 |
|---|---|
| 전제 게이트 | **2026-06-13 업데이트**: PII 215건 해소 + 운영자 수동 GT 완료 + core-2 benchmark 203 fixtures 생성. 단, 외부 teacher OCR(CLOVA/Google) 인가용 게이트는 stale summary 이슈가 있어 preflight/benchmark 산출물을 권위로 삼고 별도 수선 필요 |
...
| 순서 | 담당 | 작업 | 게이트/통과 기준 |
|---:|---|---|---|
| 0 | 데이터/라벨링 | **DONE(2026-06-13)** PII 215건 해소 + 운영자 수동 GT + core-2 benchmark 203 fixtures/split 생성 | holdout 52/test 22/train 129, leakage_check_passed |
| 1 | QA/eval | full-image 베이스라인 freeze (현 0.5598/0.5406 재확인) | scored_images·privacy 플래그 확인 |
| 2 | OCR 튜닝 | **DONE(2026-06-13)** b32 holdout + det threshold sweep | b32 macro 0.6049/micro 0.614/ingredient 0.6842로 gate fail, `text_det_box_thresh=0.4` 튜너블 권고 |
| 3 | YOLO | 섹션 검출기 재학습 → 게이트 재도전 (**현재 blocked_by_annotation_review: 미검토 bbox 205건**) | §9.2 통과, 2026-06-09 yolo26s 임시 배선 금지 |
| 4 | OCR 튜닝 | 다음 recognition 후보 export→holdout→gate (§B7). b32는 baseline 후보로 보관 | holdout `norm_edit_dis>=0.90` + acc 하한 + structured gate 동시 통과 |
...
| 역할 | 책임 | 산출물 | 당면 과제 (§9.0) |
|---|---|---|---|
| YOLO | 섹션 검출기 재학습·게이트 | weights, redacted summary, boxes JSONL | 205 bbox 운영자 리뷰·라벨 보강·재학습. 2026-06-09 yolo26s 배선 금지 |
| OCR 튜닝 | sweep, finetune, export, gate | sweep/gate JSON, inference dir | `LOCAL_OCR_TEXT_DET_BOX_THRESH=0.4` 튜너블 권고 적용 검토, b32보다 좋은 후보만 holdout 재평가 |
| 데이터/라벨링 | GT, split, consensus, optional section 정책 | sanitized manifest, split report | PII/GT/core-2 split은 완료. stale benchmark gate summary 수선은 별도 툴링 |
...
- 2026-06-13 기준 `text_det_box_thresh=0.4`만 within-sweep 일관 개선값으로 우선 후보. `combined(0.2/0.4/2.5)`는 precision 오버슈트로 채택 금지.
...
- ✅ DONE(2026-06-13): core-2 benchmark 203 fixtures/split 생성, b32 holdout 재평가, det threshold sweep 완료. b32는 `field_match`/`ingredient_recall` gate 미달이므로 production 승격 금지.
- ✅ DONE(2026-06-14): `LOCAL_OCR_TEXT_DET_BOX_THRESH=0.4`를 운영 튜너블 권고로 반영. 코드 기본값은 `None`으로 유지해 PaddleOCR upstream 기본(0.6)을 보존하고, `backend/.env.example`/`docker-compose.yml`에서 env opt-in 경로만 제공.
- ✅ DONE(2026-06-14): stale `ocr-benchmark-gate` summary 수선. `gate_supplement_ocr_benchmark.py`가 bundle summary count가 stale/invalid여도 ready GT preflight를 권위로 삼아 진행할 수 있게 보강했고, Markdown에 count source를 표시한다.
- TODO: 205 bbox 운영자 리뷰 완료 후 section detector dataset gate 재실행·A100 재학습.
