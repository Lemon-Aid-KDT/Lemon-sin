...
| `status` | `awaiting_confirmation`, `ready`, `embedded`, `deleted`, `cancelled`, `failed` |
...
- S3 또는 S3-compatible object storage smoke test
...
2. `backend/scripts/check_learning_vector_db_security.py --strict`로 pgvector extension, learning/vector table, RLS, Supabase API-role grant, raw column 부재를 확인한다.
...
- 2026-05-25 로컬 Supabase Postgres clean DB 검증:
  - `alembic upgrade head`가 `0009_harden_learning_vector_supabase_access`까지 적용됨
  - `check_learning_vector_db_security.py --strict` 통과
  - `vector_extension_schema=extensions`, learning/vector 3개 table RLS enabled, unsafe privilege 0, forbidden raw column 0
  - `supabase db advisors --local --type security --output json` 결과: `No issues found`
- 2026-05-25 기존 Supabase 원격 반영 상태:
  - 현재 worktree에는 `.supabase/config.toml`/project ref가 없어 `supabase db advisors --linked`, `supabase migration list --linked`, `supabase db push --linked --dry-run` 모두 `Cannot find project ref. Have you run supabase link?`에서 중단됨
  - 원격 반영 전 필요 조건: 기존 Supabase project link 또는 percent-encoded `--db-url` 제공
