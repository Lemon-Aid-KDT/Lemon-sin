...
| 주제 | 확인한 내용 | 설계 반영 |
| --- | --- | --- |
| PostgreSQL extension | `CREATE EXTENSION [ IF NOT EXISTS ]` 구문이 있고, extension 설치에는 권한과 서버 측 지원 파일이 필요하다. | Alembic migration은 `CREATE EXTENSION IF NOT EXISTS vector`를 별도 단계로 두고, 운영 DB 권한 preflight를 요구한다. |
...
- upgrade 첫 단계: `op.execute("CREATE EXTENSION IF NOT EXISTS vector")`
...
- `CREATE EXTENSION IF NOT EXISTS vector` 포함
