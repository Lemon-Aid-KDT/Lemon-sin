...
- 신뢰도 계산: 현재 구현은 Google Vision 응답 JSON 안의 `confidence` 값을 재귀적으로 수집해 평균을 낸다. provider가 confidence를 반환하지 않으면 `None`으로 둔다.
...
- confidence 값은 응답 JSON의 bounded `confidence` 값 평균이며, 값이 없으면 `None`이다.
