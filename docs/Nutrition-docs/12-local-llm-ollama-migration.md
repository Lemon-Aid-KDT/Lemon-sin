...
세부 구현 플랜은 `docs/Nutrition-docs/28-ollama-local-llm-connection-implementation-plan.md`를 기준으로 한다. 현 단계에서는 `OllamaSupplementParser`를 중심으로 안정화하고, 필요할 때만 얇은 transport/readiness 계층을 추가한다.
