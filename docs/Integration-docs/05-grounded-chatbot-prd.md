...
부족한 지점은 "몇 개의 수동 카드가 있는 질문만 잘 답하는 챗봇"으로 보일 수 있다는
점이다. Lemon Aid의 목표는 모든 의료/영양/복약 관련 답변이 같은 구조를 통과하는
것이다.

```text
질문 분류 -> 검수 지식 검색 -> 답변 카드 정규화 -> 안전 정책 적용 -> LLM 문장화 -> 출력 검증
```

검수된 지식이 있으면 카드가 미리 손으로 만들어진 질문처럼 구체적으로 답한다. 검수된
지식이 없으면 일반 LLM 지식으로 채우지 않고 모른다고 말한다.
...
- 카드가 미리 있는 질문만 잘 답하는 구조가 아니라, 검색된 검수 지식을 항상 답변 카드
  형태로 바꿔 답한다.
...
- "채소와 단백질을 드세요"가 아니라 오이, 양배추, 두부, 달걀처럼 바로 바꿀 수 있는
  후보를 본다.
...
- 모든 사용자-facing 답변은 하나 이상의 reviewed `AnswerCard` 또는 boundary card에서
  생성한다.
...
답변 카드는 수동 seed와 동적 정규화 결과가 같은 shape를 가져야 한다.
...
- draft, paper_candidate, deprecated, stale source는 사용자 답변 카드로 만들지 않는다.
- 카드에 없는 새 건강 사실을 LLM prompt에서 만들지 않는다.
- manual card는 seed/golden example일 뿐 coverage mechanism이 아니다.

## 9. 성공 기준

- 나트륨 저녁 질문은 국물, 소스, 장류, 가공육, 김치류 중 최소 2개 이상 조정과 구체
  채소/단백질 후보를 제공한다.
...
- 카드가 있는 몇 개 질문만 잘 답하고 나머지는 LLM 일반 지식으로 답한다.
