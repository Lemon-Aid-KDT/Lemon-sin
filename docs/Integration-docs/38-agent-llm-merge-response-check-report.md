...
1. `PR-UnknownLoop`: unknown backlog를 저장/triage하는 운영 루프를 먼저 고정한다.
2. `PR-Modularize`: `knowledge.py`, `ChatbotAgent`, `renderers.py` 분리를 시작한다.
3. `PR-LiveSmoke`: `RUN_SGLANG_SMOKE`, `RUN_POSTGRES_MIGRATION_SMOKE`, `TEST_DATABASE_URL` 기반 병합 전 자동 gate를 붙인다.
4. `PR-PolishQuality`: answerable 질문의 SGLang structured output slot 품질을 개선한다.
