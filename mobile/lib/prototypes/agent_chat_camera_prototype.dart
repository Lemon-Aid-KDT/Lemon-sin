...
class _AgentChatCameraPrototypeState extends State<AgentChatCameraPrototype> {
  ...
  Future<void> _renameRegisteredSupplement(String id) async {
    final index =
        _registeredSupplements.indexWhere((supplement) => supplement.id == id);
    if (index == -1) return;
    final name = await _showEntryEditDialog(_registeredSupplements[index].name);
    if (name == null || name.isEmpty || !mounted) return;
    setState(() {
      _registeredSupplements[index] =
          _registeredSupplements[index].copyWith(name: name);
    });
  }
    ...
    return ListView(
      ...
      children: [
        ...
        const Row(
          ...
          children: [
            Expanded(
              child: _ScoreGaugeCard(title: '오늘 분석 점수', score: 82),
            ),
            SizedBox(width: AppSpace.md),
            Expanded(
              child: _ScoreGaugeCard(title: '누적 점수', score: 76),
            ),
          ],
        ),
        const SizedBox(height: AppSpace.lg),
...
class _ScoreGaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < colors.length; i += 1) {
      ...
      canvas.drawArc(
          rect, start + segment * i + 0.025, segment - 0.05, false, paint);
    }
...
class _RoutineSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items =
        _defaultRoutineItems.where((item) => item.slot == slot).toList();
    final checkedCount =
        items.where((item) => checkedRoutineIds.contains(item.id)).length;
    return AppCard(
      elevated: selected,
      ...
      children: [
        ...
        Expanded(
          child: ListView(
            ...
            children: const [
              _MessageBubble(
                text: '오늘 아침 식단과 영양제를 같이 봐줄 수 있어?',
                mine: true,
              ),
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: mine ? AppColor.brand : AppColor.border,
          ),
