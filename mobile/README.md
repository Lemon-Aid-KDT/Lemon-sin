...
## 로컬 LLM coaching/chatbot 실행 순서

Flutter web에서 개발용 샘플 코칭 버튼과 챗봇 화면을 쓰려면 SGLang뿐 아니라 FastAPI도 켜져 있어야 합니다.
...
4. 앱에서 다음 흐름을 확인합니다.
   ...
   - Daily coaching 화면에서 `개발용 샘플로 LLM 코칭 실행`을 누릅니다.
   - Dashboard의 챗봇 카드에서 Chatbot 화면으로 이동한 뒤 질문을 보냅니다.
   - 챗봇 화면은 먼저 `/api/v1/me/privacy/consents/sensitive_health_analysis` 동의를 생성하고,
     이어서 `/api/v1/ai-agent/chat`에 질문을 보냅니다.
   - `provider`, `answerability`, reviewed source, CTA가 화면에 보이면 Flutter UI와
     FastAPI agent/chatbot 경로가 이어진 상태입니다.
