...
import 'package:lemon_aid_mobile/prototypes/agent_chat_camera_prototype.dart';
...
void main() {
  ...
  testWidgets('Supplement registration previews upload and applies to groups', (
    ...
    expect(find.text('복용 중인 영양제를 등록해두고 시간대별로 관리합니다.'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(
        find.byKey(const Key('entry-edit-name')),
        '알티지 오메가-3',
      );
      await tester.tap(find.text('수정'));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'Combined analysis follows checked breakfast and supplement only',
    (tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: AgentChatCameraPrototype()),
      );
      ...
      await tester.pumpAndSettle();
      await tester.tap(
        find.byKey(const Key('supplement-toggle-morning-omega3')),
      );
      await tester.pumpAndSettle();
      await tester.tap(find.byIcon(Icons.arrow_back_ios_new_rounded));
      await tester.pumpAndSettle();
