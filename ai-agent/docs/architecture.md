...
- `SGLangClient`: 현재 운영 후보 provider입니다. 기본 endpoint는
  `http://127.0.0.1:30000/v1`이고 `/chat/completions`를 호출합니다. 공식 문서:
  [SGLang GitHub](https://github.com/sgl-project/sglang),
  [SGLang Structured Outputs](https://docs.sglang.io/docs/advanced_features/structured_outputs).
- `OpenAICompatibleClient`: vLLM 등 대체 가능한 compatible backend용 adapter입니다.
  기본 endpoint는 `http://127.0.0.1:8000/v1`이고 `/chat/completions`를 호출합니다. 공식 문서:
...
## 2026-05-19 architecture sync

The standalone `ai-agent` package now has the same runtime boundaries that the
backend integration expects.

- LLM serving boundary:
  - Development default: `OllamaClient`.
  - Operating candidate: `SGLangClient`, using OpenAI-compatible
    `/v1/chat/completions` plus optional `response_format` for JSON Schema
    structured output.
  - Alternative compatible backend: `OpenAICompatibleClient`, including vLLM or
    another self-hosted server that follows the same API shape.
- Personalization boundary:
  - `PersonalizationContext.agent_memory` carries summarized, sanitized memory
    only.
  - `CoachingAgent` can read
    `summary_json.repeated_nutrient_patterns` and reflect repeated confirmed
    patterns in recommendation priority and rationale.
  - Raw image bytes, raw OCR text, raw LLM responses, and full prompts are not
    part of this memory context.
- App adapter boundary:
  - `context["agent_memory"]` is passed into `DailyHealthAgent`.
  - `used_tools` includes `agent_memory` only when memory summaries are present.
  - Unconfirmed OCR preview responses do not create run log records in the
    standalone adapter.
