...
첫 번째는 LLM adapter입니다. `ChatAgent`가 Ollama인지, vLLM인지, 테스트용 fake인지
...
vLLM 같은 OpenAI-compatible 서버를 호출하기 위한 client입니다.
...
API key가 없으면 `"EMPTY"`를 사용합니다. vLLM 같은 로컬 또는 내부망 서버에서는
...
운영 후보로 vLLM을 열어 둔 이유는 처리량과 serving 구성이 필요해질 때
OpenAI-compatible API 형태가 backend 통합에 유리하기 때문입니다.
...
중요한 점은 실제 Ollama나 vLLM 서버를 호출하지 않았다는 것입니다. HTTP 호출은
...
운영 또는 서버 처리량이 필요할 때 사용할 수 있는 LLM serving 후보입니다.

- [vLLM OpenAI-Compatible Server](https://docs.vllm.ai/en/latest/serving/openai_compatible_server/)

### OpenAI-compatible Chat Completions

vLLM 같은 서버가 맞추는 API 형태입니다.
...
12. ai-agent/src/lemon_ai_agent/adapters/app.py
13. ai-agent/tests/test_daily_health_agent.py
14. ai-agent/tests/test_llm_and_chat_agent.py
15. ai-agent/tests/test_app_adapter.py
```
