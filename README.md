...
| AI agent chat | 구현·검증 진행 중 | `backend/ai_agent_chat/`, `backend/Nutrition-backend/src/api/v1/ai_agent.py` | `/api/v1/ai-agent/daily-coaching`과 `/api/v1/ai-agent/chat`이 FastAPI에 연결되어 있고, `agent_memory`, Q&A knowledge policy, Ollama/SGLang provider 경로를 사용합니다. |
| Frontend | 구조 준비 | `frontend/` | 팀 통합용 프론트엔드 위치만 정의된 상태입니다. |
| Mobile | MVP 연결 진행 중 | `mobile/flutter_app/` | 현재 검증 기준 Flutter 앱은 `mobile/flutter_app`입니다. dashboard, chat, daily coaching, capture/result flow가 이 경로에서 백엔드 API와 연결됩니다. |
...
사용자 입력
  ...
  ├─ 음식 이미지/식단 정보 (후속 구현)
  └─ AI agent daily coaching / chat 입력
        │
        ▼
FastAPI Backend
  ...
  └─ backend/Nutrition-backend/src/learning/
        ...
        └─ AI Agent daily coaching / chatbot policy / agent_memory
```
...
### 4. AI Agent + Flutter web 로컬 smoke

AI Agent MVP 확인은 FastAPI, Flutter web origin, CORS 설정이 함께 맞아야 합니다.
현재 로컬 기준은 `docs/superpowers/plans/2026-05-22-mvp-runtime-and-medical-knowledge-todo.md`를 따릅니다.
...
브라우저에서 chatbot을 확인할 때는 `POST /api/v1/ai-agent/chat` 요청이 CORS 실패 없이 도달하고, 응답에 raw trace, raw findings, 내부 policy 문자열, 진단·치료·처방 단정이 노출되지 않는지 확인합니다.
