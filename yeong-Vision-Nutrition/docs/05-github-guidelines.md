...
│   │   ├── services/           # API orchestration과 유스케이스 서비스
│   │   ├── ocr/                # OCR adapter, preprocessing, provider
│   │   ├── llm/                # Ollama local LLM parser/adapter
│   │   ├── vision/             # YOLO ROI detector scaffold
│   │   ├── learning/           # 동의 기반 embedding/vector-store gate
│   │   ├── nutrition/          # KDRIs 룩업 + 결핍 진단
│   │   ├── prediction/         # 체중 예측
│   │   ├── privacy/            # 동의 정책, 보유·삭제 정책
│   │   ├── api/                # FastAPI 라우터
...
- **AI 기능은 adapter 단위 격리**: OCR·LLM·YOLO·pgvector는 `services/`에서 직접 구현하지 않고 `ocr/`, `llm/`, `vision/`, `learning/` 아래 adapter로 분리
- **기능 플래그 기본 OFF**: `enable_multimodal_llm`, `enable_vision_classifier`, `enable_image_learning_pipeline`, `enable_pgvector_storage`는 발주처/동의 게이트 전까지 OFF
