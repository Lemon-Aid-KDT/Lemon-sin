...
| 항목 | 명령 | 현재 결과 |
| --- | --- | --- |
| 전체 backend test + coverage gate | `.venv/bin/python -m pytest --cov-report=term-missing` | `310 passed, 1 skipped`, total coverage `87.67%` |
| formatting | `.venv/bin/black --check src tests scripts alembic` | pass, `166 files` |
| lint | `.venv/bin/ruff check src tests scripts alembic` | pass |
...
| regulated flag defaults | `Settings()` and readiness JSON default check | `feature_prescription_ocr_intake`, `feature_lab_result_ocr_intake`, `feature_medication_safety_alert`, `FEATURE_HOSPITAL_MOCK_FHIR` all `false` |
...
- 전체 backend test `310 passed, 1 skipped` 이상 유지
