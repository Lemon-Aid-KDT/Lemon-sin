...
스냅샷 기준: 2026-05-14 로컬 backend 재검증 결과. 상태값은 `done`, `open`, `blocked`만 사용하며, 이번 주간 범위에서 `blocked` 항목은 없다.

| 항목 | 상태 | 근거 | 남은 작업 |
| --- | --- | --- | --- |
| KDRIs 2025 전환 | `done` | `data/kdris/kdris_2025.csv` 1,795개 approved row 검증, `KDRIS_DATA_VERSION=2025`, `ALLOW_SAMPLE_KDRIS=false` | 없음 |
...
| 검증 | 명령 | 결과 |
| --- | --- | --- |
| Formatting | `.venv/bin/black --check src tests scripts alembic` | pass, 166 files unchanged |
...
이전 계획 입력의 `267 passed, 1 skipped`는 현재 코드 기준 재검증 결과와 다르므로, 이 주간 스냅샷에는 오늘 실행한 `310 passed, 1 skipped`를 실제 결과로 기록한다.
...
멀티모달 Ollama와 YOLO 실험 상세 설계는 `docs/30-multimodal-yolo-experiment-plan.md`를 따른다. 해당 플랜의 핵심 제약은 YOLO를 제품명/성분 추출 주 경로로 쓰지 않고, 영양제 병/라벨/블리스터 ROI 보조에만 쓰는 것이다.

## Learning/Vector DB Status

학습 파이프라인은 독립 모듈 골격까지 추가되어 있다.
...
- vector upsert worker 또는 API endpoint
...
`docs/previous-version/` 아래 문서는 과거 snapshot이므로 현재 구현 문서로 사용하지 않는다.
