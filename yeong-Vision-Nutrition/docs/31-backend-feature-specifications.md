...
> **문서 정보**
> 버전: v1.0 | 작성일: 2026-05-13 | 상태: 현행 구현 반영 | 작성자: yeong-tech
...
├── ocr/          ─ OCR Adapter (Google Vision · CLOVA · Noop)
...
**핵심 상수** ([bmi.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/algorithms/bmi.py)):
...
**핵심 상수** ([activity.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/algorithms/activity.py)): `BASE_STEPS=8000`, `ACHIEVEMENT_CAP=1.2`, `MAX_V1_SCORE_AT_CAP=83.33`.
...
**인터페이스** ([metabolism.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/algorithms/metabolism.py)):
...
**상수** ([weight.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/prediction/weight.py)): `KCAL_PER_KG_FAT=7700.0`, `LONG_TERM_WARNING_DAYS=90`.
...
**선택 로직** ([selector.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/prediction/selector.py)):
...
**신체구성 추정** ([body_composition.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/prediction/body_composition.py)):
...
**기술 스택**: Python `csv` 표준 라이브러리 + `lru_cache` 싱글톤, `data/kdris/kdris_2020.csv`.
...
- `kdris_manifest_path` 로 데이터 출처·검증일 메타데이터 분리 ([source_manifest.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/nutrition/source_manifest.py)).
...
**기능**: Google Vision OCR 결과가 신뢰도 낮을 때 로컬 Ollama 멀티모달 모델(Gemma 4 등)로 보조 텍스트 추출을 수행한다.

**기술 스택**: `base64` 이미지 인코딩 + Ollama messages API(`images` 필드), `ollama.py` 의 client 재사용.

**구현 결정**:
- **OCR fallback 전용**: 영양제 정보 결정에 단독 사용 금지. Google Vision 결과의 보조 채널.
...
**구현 결정**:
- **owner_subject_hash 로 PII 격리**: 사용자 이메일/ID 가 아닌 HMAC pseudonym 으로 DB 컬럼 저장 ([security/subjects.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/security/subjects.py)).
...
- `match_supplement_product` ([supplement_matching.py](03_lemon_healthcare/yeong-Vision-Nutrition/backend/src/services/supplement_matching.py)) 로 식약처 DB 자동 매칭.
...
전체: **190 passed + 1 skipped** (asyncpg 미설치 환경에서는 1건 추가 fail).
...
- Phase 1 안정화: [docs/23-p1-stabilization-plan.md](./23-p1-stabilization-plan.md)
- 백엔드 파일 구조: [docs/20-backend-file-structure-plan.md](./20-backend-file-structure-plan.md) / [docs/21](./21-backend-file-structure-guide.md)
- PostgreSQL 정식 전환: [docs/24-postgresql-transition-plan.md](./24-postgresql-transition-plan.md)
- OCR 정식 도입: [docs/25-ocr-text-supplement-analysis-plan.md](./25-ocr-text-supplement-analysis-plan.md), [docs/26-ot-s2-ocr-provider-adapter-implementation-plan.md](./26-ot-s2-ocr-provider-adapter-implementation-plan.md), [docs/27-ot-s2b-google-vision-ocr-review-plan.md](./27-ot-s2b-google-vision-ocr-review-plan.md)
- 로컬 Ollama: [docs/28-ollama-local-llm-connection-implementation-plan.md](./28-ollama-local-llm-connection-implementation-plan.md)
- Hall-lite: [docs/29-hall-lite-weight-prediction-implementation-plan.md](./29-hall-lite-weight-prediction-implementation-plan.md)
- 멀티모달·YOLO: [docs/30-multimodal-yolo-experiment-plan.md](./30-multimodal-yolo-experiment-plan.md)
- 컴플라이언스: [docs/17-image-collection-consent-plan.md](./17-image-collection-consent-plan.md), [docs/14](./14-pre-implementation-scope-and-rules.md), [docs/15](./15-regulated-feature-feasibility-and-compliance-plan.md)
...
| 날짜 | 변경 내용 | 작성자 |
| --- | --- | --- |
| 2026-05-13 | 최초 작성. 97개 .py 모듈을 17개 영역으로 정리 + 컴플라이언스·테스트 매핑. | yeong-tech |
