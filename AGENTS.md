...
> 이 문서는 **Codex가 이 프로젝트에서 어떤 작업을 하든 가장 먼저 읽어야 하는 컨텍스트**입니다.  
...
│   │   └── ... (01~29)
│   ├── superpowers/plans/       # Codex 실행 계획
│   └── deliverables/            # 데모·발표·인수인계·운영 산출물
├── backend/                     # Python 백엔드
│   ├── AGENTS.md                ← Tier 2 (백엔드 작업 시 추가 참조)
│   ├── src/
│   │   ├── algorithms/          # v1~v4, BMR, TDEE
│   │   ├── ocr/                 # OCR Adapter
│   │   ├── llm/                 # LLM Adapter
│   │   ├── nutrition/           # KDRIs 룩업, 영양 상태 평가
│   │   ├── prediction/          # 체중 예측
│   │   ├── activity/            # 활동점수
│   │   ├── api/                 # FastAPI 라우터
│   │   ├── services/            # 애플리케이션 서비스
...
├── data/                        # 정적 데이터
│   ├── AGENTS.md                ← Tier 2 (데이터 작업 시 추가 참조)
│   └── kdris_2020.csv
├── notebooks/
├── scripts/
└── .github/
...
> ⚠️ 새 최상위 폴더가 필요하면 반드시 사용자에게 먼저 확인하세요. Phase 6 산출물은 기본적으로 `docs/deliverables/` 아래에 둡니다.
...
OCR(Cloud Vision, CLOVA), LLM(Codex, GPT) 등 외부 API는 **절대 직접 호출하지 마세요**. Adapter 인터페이스를 통해야 합니다.
...
> 📖 **상세 패턴**: [backend/AGENTS.md](./backend/AGENTS.md)
...
| FastAPI 라우터 | `docs/06-tech-stack.md` | `backend/AGENTS.md` |
| DB 스키마 | `docs/06-tech-stack.md` §3.3 | `docs/09-data-catalog.md` §8 |
| KDRIs / 식약처 데이터 | `docs/09-data-catalog.md` | `data/AGENTS.md` |
...
## 🤖 Codex에게 작업 의뢰하는 표준 패턴

사용자가 Codex에 다음과 같이 작업을 의뢰할 때:
...
→ Codex는:
1. 먼저 이 `AGENTS.md` 를 읽고 프로젝트 규칙 숙지
2. `backend/AGENTS.md` 읽고 백엔드 추가 규칙 숙지 (백엔드 작업 시)
